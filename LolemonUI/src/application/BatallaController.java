...
public class BatallaController implements Initializable {
	...
	private final String VICTORYBACKROUND="-fx-background-image: url(\"/view/victory.png\");\r\n" + 
			"	-fx-background-size: stretch;\r\n" + 
			"	-fx-background-repeat: no-repeat;";
	private final String DEFEATBACKROUND="-fx-background-image: url(\"/view/defeat.png\");\r\n" + 
			"	-fx-background-size: stretch;\r\n" + 
			"	-fx-background-repeat: no-repeat;";
	@FXML
	...
	private void UsarItem(Tipo tipo) {
			if (tipo == Tipo.VIDA) {
				...
				if (combate.getTurno()) {
					...
					p1shieldimg.setImage(new Image("view/shield.png"));

				} else {
					combate.UsarVial(personaje2);
					usuarioModel.get().getInventario().getVialesList().remove(0);
					seleccionado2.get().defensaProperty().set(combate.getJ2().getDefensa());
					p2shieldimg.setImage(new Image("view/shield.png"));
				}
			}
